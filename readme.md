...
│   ├── motor.h/c                 # 电机驱动
│   ├── grayscale.c               # 8 路灰度（H55 8-wire parallel floating input）
...
| PA16 | BIN1（电机 2 方向） | |
| PA19 / PA20 | SWD（调试） | |
| PA21 | TIMA0 CCP0（旧舵机 PWM，v4.23 之前） | v4.22：原 PA0 → 改 PA15，不再用 PA21 |
| PA23 | VREF | |
...
| PB8 / PB9 | **空** | 原 DAT/CLK，现改 8-wire parallel 后空闲 |
| PB25 / PB18 / PB21 / PB22 | **空**（原 H55 灰度接线，已迁到 H56） | |
| PA30 / PB10 / PB11 / PB14 | **空**（原 H55 灰度接线，已迁到 H56） | |
| PB27 / PB26 / PB23 / PB13 | GRAY1~GRAY4 | 辅助板 CH1~CH4（floating input） |
| PB12 | GRAY5 | 辅助板 CH5（floating input） |
| PB16 / PB15 / PB5 | GRAY6~GRAY8 | 辅助板 CH6~CH8（floating input） |
| PB17 / PB19 / PB24 | AIN1 / AIN2 / BIN2 | |
| PA0 / PA1 | **MPU6050 软件 I2C**（SDA / SCL） | v4.23：换 I2C MPU6050 模块，替代原 serial U41 |
| PA15 | TIMA0_CCP2（舵机 PWM） | v4.23：从 CCP0(PA0) 搬过来，让 PA0 给 MPU6050_SDA；LQFP-64(PM) 封装上 PA15 是 CCP2（用户硬件表"PA15 TIM0_CH2"=通道 2） |
| PB8 | LED1 | v4.23：从 PA15 让位给 SERVO |
...
### ① MSPM0G3507 的 I2C 引脚复用（v4.23 更新）

PA0/PA1 **只能**复用为 I2C0（SDA/SCL），而 I2C0 已经被 OLED 占用（PA28/PA31），所以：
...
- SysConfig 在 `empty.syscfg` 里以 GPIO5 (MPU6050_SW_I2C) 暴露，**不要**去挂 I2C 实例
- v4.22 误用 I2C0 读 serial U41 模块 → 失败；v4.23 换模块后回到软件 I2C
...
- 模式：**8-wire parallel**（辅助板 V1.3 出厂默认）——8 路独立数字输出直接读
- 引脚（全部 floating input）：
  - GRAY1=PB27 / GRAY2=PB26 / GRAY3=PB23 / GRAY4=PB13
  - GRAY5=PB12 / GRAY6=PB16 / GRAY7=PB15 / GRAY8=PB5
  - 物理位置：H56 pin 1,2,3,4,5,7,9,10（跳 pin 6 = PA11 UART RX、pin 8 = PA10 UART TX）
- 接线：辅助板 CH1~CH8 → 上面 8 个 GPIO；辅助板 GND 必须接到主控 GND（共地）
- 数据：8 条 `DL_GPIO_readPins` 拼成 1 byte，bit0..7 = CH1..CH8，1=白/亮、0=黑/压线
- 写入：`car.gray_raw`（8 位打包）+ `car.gray[8]`（拆 bit）
- 周期：20ms（`SOFT_TIMER_READ_GRAY`）
